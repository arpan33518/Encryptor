...
function initUI(onSendMessage) {
  ...
  const screen = blessed.screen({
    smartCSR: true,
    title: 'P2P SSH Chat Engine (Practice)'
  });
  ...
  const header = blessed.box({
    ...
    tags: true,
    content: '{bold}{cyan-fg} P2P SSH Chat (Practice) {/cyan-fg}{/bold} | {yellow-fg}Status: Initializing...{/yellow-fg} | {gray-fg}[PgUp/PgDn: Scroll | Ctrl+C: Exit]{/gray-fg}'
  });
  ...
  const messageList = blessed.box({
    ...
    left: 0,
    border: { type: 'line' },
    tags: true,        // Enables formatting tags
    scrollable: true,
    alwaysScroll: true,
    scrollbar: {
      ch: ' ',
